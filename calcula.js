...
alert("Para ver los resultados de Binario, Octal y hexadecimal \ndebes hacer una operacion y obtener el resultado,\nluego seleccionar la funcion que desear visualizar");
...
function igualar() {
             ...
             if (op=="no") { //no hay ninguna operación pendiente.
                ...
                if (ex=="no") {
                  ...
                  x=ni; //guardamos la solución
                  op="no"; //ya no hay operaciones pendientes
                  xi=1;
                  ex="no";
                  pantalla.innerHTML=ni; //mostramos la solución
              }
          }
        }
function binario(){
...
function hexadecimal(){
                pantalla.innerHTML=x.toString(16);
...
function sen() {
         x=Math.sin(x) //resolver función seno.
         pantalla.innerHTML=x; //mostrar en pantalla resultado
         op="no"; //quitar operaciones pendientes.
         xi=1; //se puede reiniciar la pantalla 
         }
function tan() {
         x=Math.tan(x) //resolver función tangente.
         pantalla.innerHTML=x; //mostrar en pantalla resultado
         op="no"; //quitar operaciones pendientes.
         xi=1; //se puede reiniciar la pantalla 
         }
function cos() {
         x=Math.cos(x) //resolver función coseno.
         pantalla.innerHTML=x; //mostrar en pantalla resultado
         op="no"; //quitar operaciones pendientes.
         xi=1; //se puede reiniciar la pantalla 
         }
function cuadrado() {
         x=x*x; //resolver función cuadrado.
         pantalla.innerHTML=x; //mostrar en pantalla resultado
         op="no"; //quitar operaciones pendientes.
         xi=1; //se puede reiniciar la pantalla 
         }
function cubo() {
         x=x*x*x; //resolver función cubo.
         pantalla.innerHTML=x; //mostrar en pantalla resultado
         op="no"; //quitar operaciones pendientes.
         xi=1; //se puede reiniciar la pantalla 
         }
